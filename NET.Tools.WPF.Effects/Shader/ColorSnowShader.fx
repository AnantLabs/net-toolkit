...
float4 main(float2 uv : TEXCOORD) : COLOR
{
	...
	if (ret <= details)
	{
		color.r = color.r * (1 - alpha) + (rand(uv.x * uv.y * random * 1.345f) % 1.0f) * alpha;
		color.g = color.g * (1 - alpha) + (rand(uv.x * uv.y * random * 2.123f) % 1.0f) * alpha;
		color.b = color.b * (1 - alpha) + (rand(uv.x * uv.y * random * 3.001f) % 1.0f) * alpha;
	}
